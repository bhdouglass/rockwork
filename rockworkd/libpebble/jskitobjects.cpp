...
JSKitPebble::JSKitPebble(const AppInfo &info, JSKitManager *mgr, QObject *parent)
    : QObject(parent), _appInfo(info), _mgr(mgr)
{
}
...
{
    qDebug() << "Setting interval for " << delay << "ms: " << expression.toString();
...
{
    qDebug() << "Killing interval " << timerId ;
    killTimer(timerId);
...
{
    qDebug() << "Setting timeout for " << delay << "ms: " << expression.toString();
    if (expression.isString() || expression.isCallable()) {
        int timerId = startTimer(delay);
...
{
    qDebug() << "Killing timeout " << timerId ;
    killTimer(timerId);
...
void JSKitPebble::timerEvent(QTimerEvent *event)
{
    ...
    if (expression.isCallable()) { // call it if it's a function
        ...
        qDebug() << "Timer function result: " << result.toString();
    }
    else { // otherwise evaluate it
        QJSValue result = _mgr->engine()->evaluate(expression.toString());
        qDebug() << "Timer expression result: " << result.toString();
    }
}
...
{
    qDebug() << "showSimpleNotificationOnPebble" << title << body;
...
//    QString token = _mgr->_settings->property("accountToken").toString();
    if (token.isEmpty()) {
        ...
        qDebug() << "created new account token" << token;
...
//        _mgr->_settings->setProperty("accountToken", token);
    ...
    qDebug() << "returning account token" << hash;
...
QString JSKitPebble::getWatchToken() const
{
    ...
    hasher.addData(token_salt, strlen(token_salt));
    hasher.addData(_appInfo.uuid().toByteArray());
    hasher.addData(_mgr->m_pebble->serialNumber().toLatin1());

    QString hash = hasher.result().toHex();
    qDebug() << "returning watch token" << hash;
...
void JSKitPebble::invokeCallbacks(const QString &type, const QJSValueList &args)
{
    ...
    for (QList<QJSValue>::iterator it = callbacks.begin(); it != callbacks.end(); ++it) {
        qDebug() << "invoking callback" << type << it->toString();
        QJSValue result = it->call(args);
        if (result.isError()) {
            qWarning() << "error while invoking callback" << type << it->toString() << ":"
                             << JSKitManager::describeError(result);
        }
    }
}
